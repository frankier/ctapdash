...
class DescribeTests(unittest.TestCase):
    def setUp(self):
        self.info = mne.create_info(["Fz", "Cz"], sfreq=100, ch_types="eeg")

    def test_raw_returns_scipy_statistics_by_channel(self):
        ...
        self.assertEqual(dict(result.sizes), {"recording": 1, "channel": 2})
        self.assertEqual(
            list(result.data_vars),
            ["nobs", "min", "max", "mean", "variance", "skewness", "kurtosis"],
        )
        self.assertEqual(result.channel.values.tolist(), ["Fz", "Cz"])
    ...
    def test_epochs_flattens_epochs_and_times_per_channel(self):
        data = np.array(
            [
        ...
        epochs = mne.EpochsArray(data, self.info, verbose="error")
    ...
    def test_multiple_recordings_align_different_channels(self):
        raw = mne.io.RawArray(
            np.array([[1.0, 2.0], [3.0, 4.0]]), self.info, verbose="error"
        )
        ...
        self.assertEqual(result.channel.values.tolist(), ["Cz", "Fz", "Pz"])
        self.assertEqual(result.recording.values.tolist(), [0, 1])
        self.assertTrue(np.isnan(result["mean"].sel(recording=0, channel="Pz")))
        self.assertTrue(np.isnan(result["mean"].sel(recording=1, channel="Fz")))

    def test_requires_at_least_one_supported_mne_instance(self):
        with self.assertRaisesRegex(ValueError, "at least one"):
            describe()
        with self.assertRaisesRegex(TypeError, "Raw or Epochs"):
            describe(object())
