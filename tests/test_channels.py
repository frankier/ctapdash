...
def recording(
    tmp_path, *, epochs=1, names=("Fz", "Cz"), types=("EEG", "Custom"), positions=None
):
    ...
    savemat(
        path, {key: value for key, value in eeg.items() if not key.startswith("__")}
    )
...
def test_union_types_bads_and_first_usable_position(tmp_path):
    ...
    early.info["bads"] = ["Cz"]
    early.raw_montage = mne.channels.make_dig_montage(
        ch_pos={"Cz": [0, 0, 0.09]}, coord_frame="head"
    )
    late.raw_montage = mne.channels.make_dig_montage(
        ch_pos={"Fz": [0, 0.04, 0.08], "Pz": [0, -0.04, 0.08]}, coord_frame="head"
    )
    with patch(
        "ctapdash.channels._head_geometry",
        return_value=({"points": {}, "outlines": [], "message": ""}, {}),
    ) as project:
        result = channel_metadata(
            [(2, late), (1, early)], bads_by_step={2: ["Pz", "unknown"]}
        )
    ...
    np.testing.assert_array_equal(
        project.call_args.args[0].get_positions()["ch_pos"]["Fz"], [0, 0.04, 0.08]
    )
...
def test_ctap_metadata_projects_without_loading_samples(tmp_path, epochs):
    ...
    raw.raw_montage = mne.channels.make_dig_montage(
        ch_pos={name: montage[name] for name in raw.ch_names}, coord_frame="head"
    )
...
def test_missing_and_failed_positions_retain_checklist(tmp_path):
    ...
    raw.raw_montage = mne.channels.make_dig_montage(
        ch_pos={"Fz": [0.02, 0.03, 0.08]}, coord_frame="head"
    )
    with patch(
        "ctapdash.channels._head_geometry", side_effect=ValueError("bad geometry")
    ):
...
def test_heatmap_filters_before_normalizing_and_handles_empty():
    stats = xr.Dataset(
        {"mean": (("recording", "channel"), [[0.0, 10.0, 100.0], [1.0, np.nan, 90.0]])},
        coords={
            "channel": ["A", "B", "C"],
    ...
    assert (
        filtered["rows"][0]["cells"][1]["style"] != full["rows"][0]["cells"][1]["style"]
    )
...
def test_eog_positions_come_from_raw_montage(tmp_path, epochs):
    raw = recording(
        ...
        names=("VEOG", "HEOG"),
        types=("EOG", "EOG"),
        positions=[(0.08, 0.03, -0.02), (0.08, -0.03, 0.02)],
    )
    ...
    assert (
        result["head"]["front"]["points"]["VEOG"]
        != result["head"]["front"]["points"]["HEOG"]
    )
    for name, pos in before.items():
        np.testing.assert_array_equal(
            pos, raw.raw_montage.get_positions()["ch_pos"][name]
        )
...
def test_geometry_matches_mne_with_fiducials_and_headshape():
    ...
    montage = mne.channels.make_dig_montage(
        ...
        lpa=positions["lpa"],
        rpa=positions["rpa"],
        hsp=np.array(list(positions["ch_pos"].values())),
        coord_frame="head",
    )
    ...
    np.testing.assert_allclose(
        list(head["points"].values()), expected * [1, -1], atol=1e-12
    )
...
def test_montage_union_preserves_first_usable_eog_position(tmp_path):
    ...
    early.raw_montage = mne.channels.make_dig_montage(
        ch_pos={"Fz": [0, 0.04, 0.08]}, coord_frame="head"
    )
    late.raw_montage = mne.channels.make_dig_montage(
        ch_pos={"Fz": [0.01, 0.04, 0.08], "VEOG": [0.03, 0.08, -0.02]},
        coord_frame="head",
    )
    result = channel_metadata([(2, late), (1, early)])
    assert set(result["head"]["points"]) == {"Fz", "VEOG"}
    assert result["head"]["front"]["points"]["Fz"] == [0, -0.08]
...
def test_front_back_partition_and_boundary():
    ...
    positions = {
        "Front": [0.02, 0.05, 0.08],
        "Back": [0.02, -0.05, 0.08],
        "Boundary": [0.02, 0, 0.08],
    }
    head, _ = _head_geometry(
    ...
    assert head["back"]["points"]["Back"] == [0.02, -0.08]
