...
def stats_fixture():
    return xr.Dataset(
        {
            "min": (("recording", "channel"), [[-2.0, -1.0], [0.0, 1.0]]),
            "max": (("recording", "channel"), [[2.0, 1.0], [3.0, 4.0]]),
        },
        coords={
            "channel": ["Cz", "Pz"],
...
def test_shared_mapping_and_retry(tmp_path, monkeypatch):
    ...
    hurrier = SimpleNamespace(
        hurry=AsyncMock(side_effect=[RuntimeError("worker failed"), None])
    )
...
def test_unregistered_and_load_failure_retry(tmp_path, monkeypatch):
    ...
    async def run():
        with pytest.raises(RuntimeError, match="not registered"):
            await cache.get(tmp_path)
        monkeypatch.setitem(
            REGISTERED, str(tmp_path), SimpleNamespace(hurry=AsyncMock())
        )
...
def test_stats_selection_and_invalid_bounds():
    ...
    np.testing.assert_array_equal(
        _stats_extrema(stats, "p", "1", ["Pz", "Cz"]), [[-1, 1], [-2, 2]]
    )
    ...
    for value in [np.nan, np.inf, 3.0]:
    ...
    stats["min"].data[0, 0] = 2.0
    np.testing.assert_array_equal(_stats_extrema(stats, "p", 1, ["Cz"]), [[2.0, 2.0]])


@pytest.mark.parametrize(
    "destroy,error", [(False, False), (True, False), (False, True)]
)
...
def test_http_viewer_waits_for_stats(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(
        webapp,
        "participant_context",
        lambda *args, **kwargs: {"source": "s", "participant": "p"},
    )
    monkeypatch.setattr(
        ObservationData, "from_request", lambda *args, **kwargs: dataset
    )
    ...
    monkeypatch.setattr(
        webapp.templates, "TemplateResponse", lambda *args, **kwargs: kwargs["context"]
    )
    assert (
        asyncio.run(webapp.venn_time_series(request))["venn_time_series"] == "embedded"
    )
