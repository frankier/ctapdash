...
def draw(arr, ranges, overall_range):
    ...
    for x in range(width):
        ...
        for i in range(2):
            cur_ranges[i, 0] = min(
                cur_ranges[i, 0], ranges[i][start_bucket_idx:end_bucket_idx, 0].min()
            )
            cur_ranges[i, 1] = max(
                cur_ranges[i, 1], ranges[i][start_bucket_idx:end_bucket_idx, 1].max()
            )
        # print(cur_ranges)
...
def venn_time_series(eegs, width_px, height_px, time_range):
    ...
    overall_range = (
        min([c[:, 0].min() for c in coarsests]),
        max([c[:, 1].max() for c in coarsests]),
    )
