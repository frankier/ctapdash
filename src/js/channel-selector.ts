...
function stored(key: string, names: string[]): string[] {
    try {
        ...
        /* Storage may be disabled; the component remains usable. */
    }
    return names
}
...
export class ChannelSelector extends HTMLElement {
    ...
    get selectedNames(): string[] {
        return this._channels.map((c) => c.name).filter((n) => this._selected.has(n))
    }
    ...
    set availableNames(value: string[]) {
        this.availabilityInitialized = true
        this._available = new Set(value)
        this.schedule()
    }
    get badsByStep(): Groups {
        return this._bads
    }
    set badsByStep(value: Groups) {
        this._bads = value
        this.schedule()
    }
    ...
    private emit(): void {
        this.dispatchEvent(
            new CustomEvent("channel-selection-change", {
                bubbles: true,
                composed: true,
                detail: {selectedNames: this.selectedNames},
            }),
        )
    }
    private change(names: string[], selected: boolean): void {
        ...
        window.dispatchEvent(
            new CustomEvent(stateEvent, {
                detail: {key: this.key, names: selectedNames, source: this},
            }),
        )
        this.schedule()
        this.emit()
    ...
    private badDescription(name: string): string {
        const steps = Object.entries(this._bads)
    ...
    private label(name: string): string {
        return [
            name,
            this.badDescription(name),
            this._available.has(name) ? "" : this.unavailableReason,
        ]
            .filter(Boolean)
            .join(" — ")
    }
    ...
    private render(): void {
        const scrollPositions = [...this.root.querySelectorAll(".scroll-panel")].map((p) => [
            p.scrollLeft,
            p.scrollTop,
        ])
        ...
        const count = element(
            "p",
            `${this.selectedNames.filter((n) => this._available.has(n)).length} / ${this._available.size} available channels selected`,
        )
        ...
        for (const side of ["list", "diagram"]) {
            ...
            tabs.setAttribute(
                "aria-label",
                side === "list" ? "Group channels by" : "Channel diagram view",
            )
            const activate = (id: string) => {
                if (side === "list") this.grouping = id
                else this.view = id
                this.schedule()
            }
            for (const [id, name] of choices) {
                const tab = button(name, () => activate(id))
                tab.id = `tab-${id}`
                tab.dataset.focus = tab.id
                tab.setAttribute("role", "tab")
                tab.setAttribute("aria-selected", String(current === id))
                tab.setAttribute("aria-controls", `panel-${side}`)
                tab.tabIndex = current === id ? 0 : -1
                tab.onkeydown = (event) => {
                    if (["ArrowLeft", "ArrowRight", "Home", "End"].includes(event.key)) {
                        ...
                        const next =
                            event.key === "Home"
                                ...
                                : event.key === "End"
                                  ? choices[1][0]
                                  : choices.find(([key]) => key !== id)![0]
                        activate(next)
                        this.render()
                        this.root.querySelector<HTMLButtonElement>(`#tab-${next}`)?.focus()
                    }
                }
                tabs.append(tab)
            }
        ...
        this.root.querySelectorAll(".scroll-panel").forEach((panel, i) => {
            panel.scrollLeft = scrollPositions[i]?.[0] ?? 0
            panel.scrollTop = scrollPositions[i]?.[1] ?? 0
        })
        if (focusKey) {
            const replacement = [...this.root.querySelectorAll<HTMLElement>("[data-focus]")].find(
    ...
    private renderList(panel: HTMLElement): void {
        ...
        const matching = new Set(filtered.map((c) => c.name))
        ...
        if (this.grouping === "type") {
            ...
            for (const [name, names] of Object.entries(source))
                groups[name] = names.filter((n) => matching.has(n))
        ...
        for (const [group, names] of Object.entries(groups)) {
            ...
            const available = names.filter((n) => this._available.has(n)),
                selected = available.filter((n) => this._selected.has(n))
            ...
            checkbox.dataset.focus = `group:${group}`
            checkbox.onchange = () => this.change(available, checkbox.checked)
            label.append(
                checkbox,
                document.createTextNode(` ${group} (${selected.length}/${available.length})`),
            )
            legend.append(label)
            field.append(legend)
            const columns = element("div")
            columns.className = "channel-columns"
            const grouped = this.grouping === "type" ? this.typeColumns[group] : undefined
            const channelColumns = grouped?.length
                ? grouped.map((column) => column.filter((name) => matching.has(name)))
                : [names]
            ...
            for (const columnNames of channelColumns) {
                ...
                for (const name of columnNames) {
                    ...
                    row.className = "channel" + (this._available.has(name) ? "" : " unavailable")
                    row.title = this.label(name)
                    const input = element("input")
                    input.type = "checkbox"
                    input.checked = this._selected.has(name)
                    input.disabled = !this._available.has(name)
                    input.dataset.focus = `channel:${name}`
                    input.setAttribute("aria-label", this.label(name))
                    ...
                    if (bad) {
                        ...
                        badge.className = "bad"
                        badge.setAttribute("aria-label", bad)
                        row.append(badge)
                    }
                    column.append(row)
                }
    ...
    private renderHead(panel: HTMLElement): void {
        if (!Object.keys(this._head.points).length) {
            panel.append(element("p", this._head.message))
            return
        }
        const toolbar = element("div")
        toolbar.className = "toolbar"
        const mode = element("select")
        mode.setAttribute("aria-label", "Lasso action")
        mode.dataset.focus = "lasso-mode"
        for (const value of ["select", "deselect"]) {
            const option = element(
                "option",
                `${value === "select" ? "Select" : "Deselect"} enclosed sensors`,
            )
            option.value = value
            mode.append(option)
        }
        mode.value = this.lassoMode
        mode.onchange = () => {
            this.lassoMode = mode.value
        }
        toolbar.append(mode)
        panel.append(
            toolbar,
            element(
                "p",
                "Click a sensor to toggle it. Drag around sensors to select or deselect them. Channels without positions remain in the list.",
            ),
        )
        if (this.view === "front") {
            panel.append(element("h3", "Front"))
            this.renderProjection(
                panel,
                this._head.front ?? emptyHead,
                "Front sensor positions; subject’s right on left",
                "front",
            )
            panel.append(element("h3", "Back"))
            this.renderProjection(
                panel,
                this._head.back ?? emptyHead,
                "Back sensor positions; subject’s right on right",
                "back",
            )
        } else
            this.renderProjection(panel, this._head, "Head sensor positions; nose at top", "head")
    }
    private renderProjection(panel: HTMLElement, head: Head, label: string, view: string): void {
        if (!head.outlines.length && !Object.keys(head.points).length) {
            panel.append(element("p", head.message))
            return
        }
        ...
        svg.setAttribute(
            "viewBox",
            `${x} ${y} ${Math.max(...xs) - x + margin} ${Math.max(...ys) - y + margin}`,
        )
        for (const points of head.outlines) {
            ...
            line.setAttribute("stroke-width", ".001")
            svg.append(line)
        }
        for (const [name, [px, py]] of Object.entries(head.points)) {
            const sensor = document.createElementNS(ns, "g")
            sensor.classList.add("sensor")
            if (this.badDescription(name)) sensor.classList.add("bad")
            sensor.setAttribute("role", "checkbox")
            sensor.setAttribute("aria-label", this.label(name))
            sensor.setAttribute("aria-checked", String(this._selected.has(name)))
            sensor.setAttribute("aria-disabled", String(!this._available.has(name)))
            sensor.setAttribute("tabindex", this._available.has(name) ? "0" : "-1")
            sensor.dataset.focus = `sensor:${view}:${name}`
            const title = document.createElementNS(ns, "title")
            title.textContent = this.label(name)
            const circle = document.createElementNS(ns, "circle")
            circle.setAttribute("cx", String(px))
            circle.setAttribute("cy", String(py))
            circle.setAttribute("r", ".003")
            const text = document.createElementNS(ns, "text")
            text.setAttribute("x", String(px + 0.004))
            text.setAttribute("y", String(py))
            text.textContent = name + (this.badDescription(name) ? " ⚠" : "")
            sensor.append(title, circle, text)
            sensor.onclick = () => this.change([name], !this._selected.has(name))
            sensor.onkeydown = (event) => {
                if ([" ", "Enter"].includes(event.key)) {
                    event.preventDefault()
                    this.change([name], !this._selected.has(name))
                }
            }
            svg.append(sensor)
        }
        ...
        trace.setAttribute("fill", "#2563eb22")
        trace.setAttribute("stroke", "#2563eb")
        trace.setAttribute("stroke-width", ".001")
        trace.setAttribute("pointer-events", "none")
        svg.append(trace)
        const point = (event: PointerEvent): number[] => {
        ...
        svg.onpointermove = (event) => {
            if (polygon) {
                ...
                trace.setAttribute("points", polygon.map((p) => p.join(",")).join(" "))
            }
        }
        svg.onpointerup = (event) => {
            ...
            const enclosed =
                polygon.length >= 3
                    ? Object.entries(head.points)
                          .filter(([, p]) => inside(p, polygon!))
                          .map(([name]) => name)
                    : []
            polygon = null
            trace.setAttribute("points", "")
            svg.releasePointerCapture(event.pointerId)
            this.change(enclosed, this.lassoMode === "select")
        }
        svg.onpointercancel = () => {
            polygon = null
            trace.setAttribute("points", "")
...
if (!customElements.get("channel-selector"))
