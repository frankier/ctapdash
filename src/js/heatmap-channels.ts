    ...
    connectedCallback(): void {
        ...
        const update = () => {
            ...
            input.value = JSON.stringify(
                selector.selectedNames.filter((name) => selector.availableNames.includes(name)),
            )
        }
        update()
        step.addEventListener("change", update, {signal, capture: true})
        this.querySelector<HTMLButtonElement>("[data-open]")!.addEventListener(
            "click",
            () => dialog.showModal(),
            {signal},
        )
        this.querySelector<HTMLButtonElement>("[data-close]")!.addEventListener(
            "click",
            () => dialog.close(),
            {signal},
        )
        selector.addEventListener(
            ...
            () => {
                ...
                document
                    .querySelector("#channel-statistics-table")
                    ?.dispatchEvent(new Event("channels-changed", {bubbles: true}))
            },
            {signal},
        )
    }
...
if (!customElements.get("heatmap-channels"))
    customElements.define("heatmap-channels", HeatmapChannels)
