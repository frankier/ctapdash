...
def _scrollbar(plot_range, bounds, orientation, min_interval=None):
    ...
    scrollbar = RangeSlider(
        ...
        tooltips=False,
        bar_color="#d1d5db",
        width=28 if vertical else 100,
        height=100 if vertical else 28,
        margin=0,
        sizing_mode="fixed",
        stylesheets=[
            InlineStyleSheet(
...
def navigation_frame(plot, x_bounds, y_bounds, min_interval=None):
    ...
    channel_axis = figure(
        name="channel-minimap",
        x_range=(0, 1),
        y_range=Range1d(*y_bounds),
        width=65,
        height=100,
        frame_width=0,
        frame_height=100,
    ...
    layout = column(
        ...
        spacing=0,
        margin=0,
        min_height=plot.min_height + 73,
        sizing_mode="stretch_both",
    )
    align = CustomJS(
        name="align-range-navigation",
        args=dict(
            plot=plot,
