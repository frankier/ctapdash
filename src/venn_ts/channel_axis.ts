...
export class ChannelAxisView extends LinearAxisView {
    ...
    protected _draw_major_labels(ctx: Context2d, extents: Extents, ticks: TickCoords): void {
        ...
        for (let i = 0; i < values.length; i++) {
            const text =
                labels instanceof Map
                    ? (labels as Map<number | string, unknown>).get(values[i])
                    : labels[values[i]]
            ...
            if (room < 12) continue
            const rotated =
                !this.model.truncate_labels && is_channel && ctx.measureText(text).width > threshold
            const lines: string[] = []
            if (!rotated) {
                let line = text
                if (this.model.truncate_labels && ctx.measureText(line).width > 48) {
                    const chars = Array.from(line)
                    while (chars.length > 0 && ctx.measureText(chars.join("") + "...").width > 48)
                        chars.pop()
                    line = chars.join("") + "..."
                }
                lines.push(line)
                ...
                while (rest.length > 0 && lines.length < 3) {
                    let line = ""
                    while (rest.length > 0 && ctx.measureText(line + rest[0]).width <= room)
                        line += rest.shift()
                    if (lines.length == 2 && rest.length > 0) {
                        while (line.length > 0 && ctx.measureText(line + "...").width > room)
                            line = line.slice(0, -1)
                        line += "..."
                    }
                    lines.push(line)
            ...
            const y = this.model.avoid_overlap
                ? Math.max(frame.top + height / 2, Math.min(frame.bottom - height / 2, ys[i]))
                : ys[i]
        ...
        if (this.model.avoid_overlap && candidates.length > 0) {
            ...
            const channels = ordered.filter((label) =>
                this.model.channel_labels.has(values[label.index]),
            )
            const endpoints = channels.length > 0 ? channels : ordered
            selected = []
            const fits = (label: Label) =>
                selected.every(
                    (other) => Math.abs(label.y - other.y) >= (label.height + other.height) / 2 + 4,
                )
            ...
            const guides = ordered.filter(
                (label) => !this.model.channel_labels.has(values[label.index]),
            )
        ...
        for (const label of selected) {
            ...
            if (label.rotated) {
                ...
                label.lines.forEach((line, j) =>
                    ctx.fillText(line, 0, (j - (label.lines.length - 1) / 2) * 14),
                )
            } else {
...
export interface ChannelAxis {
    channel_labels: Map<number, string>
