...
export class ChannelSelectorView extends WidgetView {
  ...
  connect_signals(): void {
    ...
    this.on_change(value, () => { if (this.selector) this.selector.selectedNames = this.model.value })
  }
  private configure(): void {
    this.selector?.configure(JSON.parse(this.model.metadata_json), this.model.available, JSON.parse(this.model.bads_json))
  }
  render(): void {
    super.render()
...
export class ChannelSelector extends Widget {
  ...
  static {
    ...
    this.define<ChannelSelector.Props>(({Str, List}) => ({
      metadata_json: [Str, "{}"], available: [List(Str), []],
      bads_json: [Str, "{}"], value: [List(Str), []],
    }))
  }
}
