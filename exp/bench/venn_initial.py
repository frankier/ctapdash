    ...
    try:
        for i in range(args.runs):
            ...
            scope = dict(
                ...
                method="GET",
                scheme="http",
                path="/venn-time-series/autoload.js",
                raw_path=b"/venn-time-series/autoload.js",
                root_path="",
            ...
            while perf_counter() < deadline:
                ...
                if sessions:
                    ...
                    plots = [
                        m
                        for m in models
                        if isinstance(m, renderer.VennTimeSeriesRenderer)
                    ]
                    if len(plots) == 1:
            ...
            models = list(session.document.models)
            plots = [
                m for m in models if isinstance(m, renderer.VennTimeSeriesRenderer)
            ]
            assert len(plots) == 1, (
                f"Expected an actual plot, not an error container: {[type(r).__name__ + ':' + getattr(r, 'text', '')[:300] for r in session.document.roots]} timings={timings}"
            )
            assert plots[0].tile_requests == 0
            result = {
                "run": i + 1,
                "total_s": total,
                "phases_s": {k: sum(v) for k, v in timings.items()},
        ...
        out_path = (
            _ROOT
            / f"exp/bench/results/{('profiled' if args.profile else 'timings')}.json"
        )
